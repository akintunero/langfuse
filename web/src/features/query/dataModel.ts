...
export const observationsView: ViewDeclarationType = {
  ...
  measures: {
    ...
    timePerOutputToken: {
      sql: "if(isNull(@@AGG1@@(observations.completion_start_time)) OR isNull(@@AGG1@@(observations.end_time)) OR arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'output') > 0, @@AGG1@@(observations.usage_details)))) = 0, CAST(NULL AS Nullable(Float64)), date_diff('millisecond', @@AGG1@@(observations.completion_start_time), @@AGG1@@(observations.end_time)) / arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'output') > 0, @@AGG1@@(observations.usage_details)))))",
      aggs: { agg1: "any" },
      alias: "timePerOutputToken",
      type: "decimal",
      description: "Average time per output token during streaming.",
      unit: "millisecond",
    },
    tokensPerSecond: {
      sql: "if(isNull(@@AGG1@@(observations.completion_start_time)) OR isNull(@@AGG1@@(observations.end_time)) OR date_diff('millisecond', @@AGG1@@(observations.completion_start_time), @@AGG1@@(observations.end_time)) = 0, CAST(NULL AS Nullable(Float64)), arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'output') > 0, @@AGG1@@(observations.usage_details)))) / (date_diff('millisecond', @@AGG1@@(observations.completion_start_time), @@AGG1@@(observations.end_time)) / 1000))",
      aggs: { agg1: "any" },
      alias: "tokensPerSecond",
      type: "decimal",
      description: "Tokens generated per second during streaming.",
      unit: "tokens/second",
    },
    timeBetweenTokens: {
      sql: "if(isNull(@@AGG1@@(observations.completion_start_time)) OR isNull(@@AGG1@@(observations.end_time)) OR arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'output') > 0, @@AGG1@@(observations.usage_details)))) = 0, CAST(NULL AS Nullable(Float64)), date_diff('millisecond', @@AGG1@@(observations.completion_start_time), @@AGG1@@(observations.end_time)) / arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'output') > 0, @@AGG1@@(observations.usage_details)))))",
      aggs: { agg1: "any" },
      alias: "timeBetweenTokens",
      type: "decimal",
      description: "Average time between tokens during streaming.",
      unit: "millisecond",
    },
    countScores: {
      sql: "uniq(scores.id)",
